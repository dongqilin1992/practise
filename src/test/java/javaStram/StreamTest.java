...
public class StreamTest {
    public static void main(String[] args) {
        ...
        System.out.println("非空字符串："+filtered);
        String mergedString = strings.stream().filter(string -> !string.isEmpty()).collect(Collectors.joining(", "));
        System.out.println("合并字符串: " + mergedString);
        //limit 方法用于获取指定数量的流
        //sorted 方法用于对流进行排序
        Random random = new Random();
        random.ints().limit(10).sorted().forEach(i -> System.out.println("排序啊:" + i));

        List<Integer> numbers = Arrays.asList(3, 2, 2, 3, 7, 3, 5);
        // 获取对应的平方数
        //map 方法用于映射每个元素到对应的结果
        List<Integer> squaresList = numbers.stream().map( i -> i*i).collect(Collectors.toList());
        System.out.println("求平方:"+squaresList);
        ...
        List<String> st = list.stream().
                map(Student::getName).limit(5).skip(3).collect(Collectors.toList());
        System.out.println("五个跳过前三个："+ st);
        List<Student> a = list.stream().filter(student -> student.getName().equals("A")).collect(Collectors.toList());
        System.out.println("name=A:"+a);

        list.stream().sorted(Comparator.comparing(Student::getHeight).reversed()).forEach(student -> System.out.println("按Height排序：" + student));
        list.stream().sorted((p1, p2) ->
                p1.getName().compareTo(p2.getName())).limit(4).forEach(student -> System.out.println("按name排序：" + student));
    ...
    public void testOptional(){
        String text = null;
        Optional.ofNullable(text).ifPresent(s -> System.out.println(s.toString()));
